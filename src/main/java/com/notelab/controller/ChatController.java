...
public class ChatController {
    ...
    public void chat(@RequestBody ChatReq req, HttpServletRequest request, HttpServletResponse response)
        ...
        Map<String, Object> conv = ConversationDao.getConversation(req.conversation_id, AuthUtil.userId(user));
        ...
        ConversationDao.addMessage(cid, "user", message);
        if (ConversationDao.countMessages(cid) == 1) {
            ConversationDao.setConversationTitle(cid, message.substring(0, Math.min(30, message.length())));
        }
        ConversationDao.touchConversation(cid);
        ...
        if (full.length() > 0) {
            ConversationDao.addMessage(cid, "assistant", full.toString());
            ConversationDao.touchConversation(cid);
        }
        sendSafe(out, Map.of("done", true));
    }
